...
<html lang="en">
    ...
    <head>
        <title>Danh sách nhân viên bị xóa | Quản trị Admin</title>
        <meta charset="utf-8">
        ...
        <body class="app sidebar-mini rtl" onload="time()">
            ...
            <main class="app-content">
                <div class="app-title">
                    <ul class="app-breadcrumb breadcrumb side">
                        <li class="breadcrumb-item active"><a href="listdeletestaff"><b>Danh sách người dùng đã xóa</b></a></li>
                    </ul>
                    <div id="clock"></div>
                </div>
                <div class="row">
                        ...
                        <div class="tile">
                            <div class="tile-body">
                                <div class="row element-button">
                                    <div class="col-sm-2">
                                        <a class="btn btn-add btn-sm" href="adminstaff" title="Danh sách"><i class="fa fa-list-alt"></i>
                                ...
                                <div class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-12 col-md-6">
                                            <form id="frm" action="adminstaff"> 
                                                <div class="input-group mb-3">
                                                    <input type="hidden" name="entryperpage" value="${entryperpage}">
                                                    <input type="text" class="form-control input-text" placeholder="Nhập tên hoặc mã người dùng hoặc địa chỉ hoặc email" aria-label="Recipient's username" aria-describedby="basic-addon2">
                                    ...
                                    <form action="undostaff" method="post">
                                        <button type="submit" class="btn btn-primary" name="btno" value="0">Hoàn tác đã chọn</button>
                                        <button type="submit" id="deleteSelected" class="btn btn-undoall" name="btna" value="1">Hoàn tác tất cả</button>
                                        ...
                                        <table class="table table-hover table-bordered table-responsive" id="sampleTable">
                                            <thead>
                                                <tr style="background: #eee">
                                                    <th>Tick để xoá</th>
                                                    <th>Mã nhân viên</th>
                                                    <th style="width: 7%">Tên nhân viên</th>
                                            ...
                                            <tbody>
                                                <c:forEach items="${data}" var="item">
                                                    <tr>
                                                        <td width="10"><input type="checkbox" name="checkBox" value="${item.getAdminID()}"></td>
                                                        ...
                                                        <td>
                                                            <button class="btn btn-undo btn-sm undo mr-1" type="button" title="Hoàn tác"
                                                                    onclick="undoStaff('${item.getAdminID()}', '${page}', '${key}')""><i class="fa fa-undo"></i> 
                                                            </button>

                                                            <!--                                                        <button class="btn btn-undo btn-sm undo mr-1" id="undo" type="button" title="Hoàn tác"
                                                                                                                            data-id="${item.getAdminID()}" data-page ="${page}"
                                                                                                                            data-num ="${num}" data-key ="${key}" data-entryperpage ="${entryperpage}"><i class="fa fa-undo"></i> 
                                                                                                                    </button>-->
                                                            <button class="btn btn-primary btn-sm edit" type="button" title="Sửa" id="show-emp"
                                                                    onclick="updateStaff('${item.getAdminID()}', '${page}', '${key}')"><i class="fas fa-edit"></i></button>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
                                            </tbody>
                                            ...
                                            <div class="col-sm-12 col-md-7">
                                                <div class="dataTables_paginate paging_simple_numbers" id="sampleTable_paginate">
                                                    <ul class="pagination1">
                                                        ...
                                                        <c:if test="${page != 1}">
                                                            <li class="paginate_button page-item previous disabled" id="sampleTable_previous">
                                                                <a href="/SWP391_LapTop/listdeletestaff?page=${page-1}&key=${key}&entryperpage=${entryperpage}&num= ${num}" 
                                                        ...
                                                        <c:forEach begin="1" end="${num}" var="i">
                                                            ...
                                                            <c:if test="${i != page}">
                                                                <li class="paginate_button page-item active">
                                                                    <a href="/SWP391_LapTop/listdeletestaff?page=${i}&key=${key}&entryperpage=${entryperpage}&num= ${num}"
            ...
            <script type="text/javascript">
                                                                        ...
                                                                        function submit(page, num, key) {
                                                                            ...
                                                                            window.location.href = "/SWP391_LapTop/listdeletestaff?entryperpage=" + entryperpage.value + "&num=" + num + "&key=" + key;

                                                                        }
                                                                        function updateStaff(id, page, key) {
                                                                            var entryperpage = document.getElementById('entryperpage');
                                                                            window.location.href = "/SWP391_LapTop/updatestaff?entryperpage=" + entryperpage.value + "&page=" + page + "&key=" + key + "&id=" + id;
                                                                        }
                                                                        function undoStaff(id, page, key) {
                                                                            var entryperpage = document.getElementById('entryperpage');
                                                                            swal({
                                                                                title: "Cảnh báo",
                                                                                text: "Bạn có chắc chắn là muốn hoàn tác tài khoản này?",
                                                                                buttons: ["Hủy bỏ", "Đồng ý"],
                                                                            })
                                                                                    .then((willDelete) => {
                                                                                        if (willDelete) {
                                                                                            // Chuyển hướng URL với ID sản phẩm
                                                                                            window.location.href = "/SWP391_LapTop/undostaff?entryperpage=" + entryperpage.value + "&page=" + page + "&key=" + key + "&id=" + id;
                                                                                        }
                                                                                    });
