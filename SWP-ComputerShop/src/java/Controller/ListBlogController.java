...
public class ListBlogController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("abcsadasdas" + CateID);
        request.setCharacterEncoding("utf-8");
                int total = new BlogDao().countListBlogsPaging(search);
        ...
        if (CateID != null && CateID.equals("all")) {
          request.setAttribute("list", new BlogDao().getListBlogsPaging(search, index));
        } else {
         request.setAttribute("list", new BlogDao().getListBlogsPagingByCate(search, index,CateID));
        }
        request.setAttribute("bcl", bcl);
        request.getRequestDispatcher("list-blog-admin.jsp").forward(request, response);
    }
